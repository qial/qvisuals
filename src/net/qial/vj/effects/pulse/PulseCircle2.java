...
public class PulseCircle2 extends PulseCircle1 {
	...
	public void play() {
		...
		int outsideCircleStroke = (int) (255 * DrawUtil.waveUp(currentFrame,
				mySeq.getPulseLength()));
		...
		for (int i = 0; i < getParam(amt); i++) {
			...
			if (i == getParam(amt) - 1) {
				float brightness = DrawUtil.waveDown(
						pulse * mySeq.getPulseWidth(), mySeq.getPulseWidth());
